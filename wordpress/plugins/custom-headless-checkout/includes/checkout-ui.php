<?php
/**
 * Branded blank checkout UI (templates, assets, field tweaks).
 *
 ...
 */
function custom_headless_checkout_should_load(): bool {
...
add_filter(
	...
	static function ( $template ) {
		if ( ! custom_headless_checkout_should_load() ) {
			return $template;
		}

		$blank = CUSTOM_HEADLESS_CHECKOUT_DIR . 'template-checkout.php';
...
add_filter(
	...
	static function ( $template, $template_name, $template_path ) {
		...
		$override = CUSTOM_HEADLESS_CHECKOUT_DIR . 'woocommerce/' . $template_name;
...
add_action(
	...
	static function (): void {
		...
		wp_enqueue_style(
			'custom-headless-checkout-fonts',
		...
		$css_path = CUSTOM_HEADLESS_CHECKOUT_DIR . 'checkout.css';
		$js_path  = CUSTOM_HEADLESS_CHECKOUT_DIR . 'checkout.js';
		...
		wp_enqueue_style(
			...
			CUSTOM_HEADLESS_CHECKOUT_URL . 'checkout.css',
			array( 'custom-headless-checkout-fonts' ),
			$css_ver
		);

		$custom_css = custom_headless_checkout_get_settings()['custom_css'];
		if ( $custom_css !== '' ) {
			wp_add_inline_style( 'custom-headless-checkout', $custom_css );
		}

		wp_enqueue_script(
			'custom-headless-checkout',
			CUSTOM_HEADLESS_CHECKOUT_URL . 'checkout.js',
			array(),
			$js_ver,
			true
		);
...
add_filter(
	...
	static function ( $show ) {
		return custom_headless_checkout_should_load() ? false : $show;
	}
);
...
add_filter(
	...
	static function ( $fragments ) {
		if ( ! custom_headless_checkout_should_load() || ! WC()->cart ) {
			return $fragments;
		}
...
add_filter(
	...
	static function ( $fields ) {
		...
		if ( custom_headless_checkout_company_fields_enabled() ) {
 ...
 */
function custom_headless_checkout_is_valid_nip( string $nip ): bool {
...
add_action(
	...
	static function (): void {
		if ( ! custom_headless_checkout_should_load() || ! custom_headless_checkout_company_fields_enabled() ) {
			return;
		}
		...
		if ( ! is_string( $nip ) || ! custom_headless_checkout_is_valid_nip( $nip ) ) {
...
add_action(
	...
	static function ( $order ): void {
		if ( ! $order instanceof WC_Order || ! custom_headless_checkout_company_fields_enabled() ) {
			return;
		}
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		if ( 'woocommerce' !== $domain || ! custom_headless_checkout_should_load() ) {
			return $translation;
		}
